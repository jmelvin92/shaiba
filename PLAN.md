...
| 6.7 | Ocean biome — the western sea | `feature/ocean-biome` | 🟡 Built 2026-08-17 — coast terrain, sea surface, knee-deep wading, wet-sand prints, surf-bed hook all in; verify_ocean (all modes) + full battery green, 120 fps at the vista. **Awaiting Joshua: playtest, water-color ladder pick, screenshot review — ⚠️ nothing pushed to GitHub until he signs off** |
...
- [ ] Water look approved from the ladders; all new colors added to ART_DIRECTION's palette table with hex values + `resources/palette/` materials; no off-palette color anywhere in the biome. *(Build status: the ocean uses **zero new colors** — oasis_teal shallow / night_blue deep / plaster foam / sand_shadow wet sand, documented in ART_DIRECTION. A four-rung water-color ladder is rendered and waiting for Joshua's pick; if he picks an off-palette rung it goes through the palette-amendment rule first.)*
...
- [ ] Wading feels right to Joshua: entering the water slows you believably, the knee-deep stop is gentle (no wall-bump, no jitter, no getting trapped where waves meet sand), and walking the surf line is comfortable. *(Measured side is done — held at exactly 0.40 m with 0.06 m cross-shore drift over ~30 s of pressing seaward, along-shore walking free, walks straight back out. The feel is his to judge.)*
