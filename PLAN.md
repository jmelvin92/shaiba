...
| 6.8 | The sand worm — desert mob | `feature/sand-worm` | 🟡 Parts 1–2 ✅; Part 3 **built 2026-08-23** — noise-woken hunt (WormDirector), telegraphed escape beat, swallow→reload death, threat score in (Joshua's growls + hunt drums; ambient pauses). All 46 gate checks + full battery green; director on **playtest dials** (wake ≈ 25 s of running). **Open: Joshua's hunt playtest** — feel tuning + look sign-off close the phase. Merges after 6.7 |
| 6.9 | Character animation & the death scene | *(next session — branch when planned)* | 🔲 **Queued by Joshua 2026-08-23: the next session.** Improve player animation, and give being swallowed a real death animation — possibly blood. See the queue note under Phase 6.8 Part 3 |
...
**Part 1 exit bar**
- [x] `verify_worm` passes: raise stamps land where the worm swims and read back from the texture; the mound settles to zero after passage; the wander path over a long sample never violates a guardrail (shallow sand / homestead pad / sea); dormant cost is zero (no worm ⇒ no deformation passes, no processing); perf with the mound churning stays at the phase baseline. *(All 21 checks green 2026-08-22: 180 sim-s wander held sand ≥ 0.56 m / homestead clearance 35.7 m / sea untouched; worst pass 0.32 ms vs the 1 ms budget; the F7 summon is tested through the real InputMap.)*
- [x] `verify_prints` (the whole Phase 5 gate) and the ocean `--wetprints` mode re-run green — prints provably untouched by the raise channel. *(Both green — and the exercise found two pre-existing latent bugs, fixed and recorded in DECISIONS: the `_content_until` overwrite freeze and the wetprints test's 10×-long time_scale wait. Wetprints now passes against a live decaying texture, wet 0.65 vs dry 0.33.)*
- [x] The traveling mound reads at the gameplay camera in screenshots — a moving swell of sand, not a glitch and not a wall. *(`mound_live_1/2.png`: the swell moves between frames two seconds apart and the wake collapses behind it. Three shader rounds to get here — coreless brush, soft saturation, raise floor — see DECISIONS. Joshua judges the feel live with F7.)*
- [x] Size ladder shot from a fixed vantage; **Joshua picks the worm scale** — the pick closes Part 1 and sets Part 2's rig dimensions. *(Picked 2026-08-22: **C — the titan, 26 m long × 3.4 m body**. Worm and mound defaults updated to the rung (mound radius 3.8 m, spacing 1.5 m, lift 0.9 m, spine 2.5 m down), verify_worm re-run green. In the same breath he set the Part 3 kill: **the worm swallows the player whole**.)*
- [x] Zero warnings; standalone scene runs clean; PLAN/DECISIONS updated. *(`--import`, per-script `--check-only` ×8, world/graybox/sand_worm standalone runs all clean; ARCHITECTURE gained the mound-channel and worm sections.)*

**Handoff notes (2026-08-22, end of the Part 1 build session)**

Everything measurable is green; the one open item is **Joshua's scale pick** from the ladder. How to see it live: F7 summons/dismisses the worm near the player (F8 cycles wander / orbit / approach — orbit is the best mound demo), F3 shows its state line. All feel values are exports on `SandWorm` (speed 5.5, turn rate 55°/s, mound radius 2.4) and `SandDeformation`'s Mounds group (height 0.5 m, settle 1.4 s, churn tint 0.45); the committed defaults are my eye, not his — expect a live tuning round like the audio sessions. Worth knowing: the worm needs ≥ 0.5 m of sand and refuses the homestead surround, so summoning from the courtyard places it beyond the pad ring — walk east into the dunes for the close encounter. The `worm_rumble_loop` sourcing row is in `assets/audio/README.md` (silent until it lands). One false lead recorded so nobody rechases it: the "patchwork" visible on flat deep-sand basins in some shots is the Phase 4 sand-tone dither, not the wake.

### Part 2 — The body (started 2026-08-22)
**Rig contract from the Part 1 pick (2026-08-22): the worm is 26 m long × 3.4 m body diameter — rung C, the titan.** **Design source changed at Joshua's direction (2026-08-22): no Meshy — the model is hand-built by committed script**, from his reference photo (a Dune-style worm: shingled ring plates, head slightly wider than body, tri-lobed mouth opening around concentric teeth rings, dark gullet — `docs/references/worm_reference.png`). The body is driven **procedurally**: it follows the head's burrow path and the bones bend to match, so breaches read correctly at any angle and speed with no baked-clip seams. The worm at this stage is ambient spectacle — rare, harmless, unforgettable.

**Part 2 deliverables (planned 2026-08-22, build started same day)**
- **`tools/build_worm.py`** (committed headless Blender script, the established pipeline): the titan at exactly 26 × 3.4 m — tapered stack of overlapping ring plates (rigid vertex groups, one per ring: plates hinge at joints like armor, no soft skinning to fight the flat-shaded style), head with three mouth-lip lobes on their own bones, teeth rings, dark throat. Palette materials only; ≤ 6,000 tris (signature-creature budget, recorded here). Exports `assets/blender/worm.blend` + `assets/models/worm.glb` per the ART_DIRECTION checklist.
- **Procedural spine driver** in `sand_worm.gd`: a ring buffer of the head's path (position + tangent per ~0.5 m of travel); each spine bone sits at its arc-distance behind the head and orients along the path. The dormant worm stays hidden and free; summon pre-fills the history so the body never collapses to a point.
- **The breach**: a `breach()` maneuver (debug F9) that lifts the *head's path* into an arc above the surface and back under — the body follows through the same gate for free. Mouth lips open with breach height (the swallow mouth earns its bones now). Sand burst at the entry and exit crossings: raise stamps + a one-shot palette-sand particle burst; breach audio hooks silent-safe (`creatures/worm_breach_01`, rumble swell) — `assets/audio/README.md` grows the rows.
- **`verify_worm` grows the body checks**: bone chain tracks the path at segment spacing; the whole body stays under the surface while swimming; a breach sends the head above the surface and back, opens the mouth, stamps the crossings, and returns to clean swimming; the F9 binding tested for real; dormant stays zero-cost and every Part 1 check stays green.
...
### Part 3 — The threat (planned 2026-08-23 with Joshua; build started same session)
Noise attraction via `noise_made` (footstep loudness already varies by gait — running calls it, sneaking doesn't); a rarity/encounter director (incredibly rare, tunable — never a spawn timer the player can feel); stalk → mound-approach → strike behavior; **the kill: the worm swallows the player whole** (Joshua's call, 2026-08-22 — made with the titan pick, and exactly what a 26 m body is for). Feel items (fear pacing, fairness of the noise rule, rarity) gate on his playtests, as ever.

**Planning calls (Joshua, 2026-08-23) — the session opened with the queued conversation ("when it appears, when it attacks, how to avoid"):**
- **Death: instant kill with one escape beat.** Being caught is a cinematic swallow — dragged under, fade to black, **reload last save** (the 6.6 save system is the respawn path; no health bar, no damage numbers — the health/damage layer stays a future phase). But the strike always telegraphs: sand erupts at the committed target point for a beat before the worm arrives, and a sharp player who sprints clear survives. Death is never undodgeable.
- **Appearance: noise wakes it.** The desert is empty until the player's own noise accumulates — a hidden attraction meter fed by `noise_made`, decaying during quiet play. Rarity emerges from behavior, not a timer: a quiet, watchful player might never meet it. After an encounter ends, a calm-down cooldown keeps the next one distant.
- **Night: identical rules day and night.** Noise is noise; the properly-dark night is scary enough because the mound can't be seen coming. No night multipliers.
- **Avoidance: all of it.** Going silent (stop / crouch-sneak — crouching is already genuinely silent) makes it lose you; **safe ground** (packed earth, thin sand, the homestead pad, the beach) is physically worm-proof and becomes the survival skill; outrunning it in a straight line fails (7.5 vs 4.9 m/s) but sprinting the last stretch to nearby cover is a viable, heart-pounding play. **Throwable noisemakers (rocks now; radios, jukeboxes, fire alarms later) are planned-for, not built now** — the noise contract stays generic (any emitter of `noise_made(position, loudness)` can be connected to the director) so future sources drop in with one signal and one connect.

**Part 3 deliverables**
- **`scenes/world/worm_director.gd`** (`WormDirector`, node in the world scene — no autoload): the encounter director. Listens to `noise_made`, gates each event by sand depth at the noise position (noise on packed ground / the pad / thin sand does not carry — the courtyard is silent to the worm by physics, which is also why living there is safe), accumulates the attraction meter with per-second decay, wakes the worm beyond a spawn distance when it crests, forwards fresh noise to a hunting worm, and enforces the post-encounter calm cooldown. All gains/thresholds/distances exported.
- **Hunt state machine in `sand_worm.gd`**: SEEK (swim to last-heard position) → PROWL (circle it, lose interest on silence) → STALK (circle the prey, tightening with each fresh noise — the mound and rumble ARE the telegraph) → STRIKE (commit to the prey's position at commit-time, telegraph eruption at that point, breach mouth-first onto it — a captured point, never homing, so displacement dodges) → DEPART (swim away, despawn beyond a distance, zero cost again). Strike commit requires the target point to be swimmable — safe ground can never be struck. Debug modes (F7/F8/F9) survive unchanged.
- **The kill**: worm emits `swallowed(prey)`; LevelRoot runs the death sequence — player control cut, dragged under with the diving head, screen fade to black (**new reusable `ScreenFade`**, `scenes/ui/screen_fade/`), then reload last save (or re-seat at the homestead spawn when no save exists), fade back in. Player gains a minimal `set_control_enabled`.
- **Audio, silent-safe per 6.6**: telegraph eruption + strike one-shots join `assets/audio/README.md` (`creatures/worm_telegraph_01`, `creatures/worm_strike_01`); the existing rumble/breach rows still awaiting sourcing.
- **F3 debug**: director line (attraction %, state, cooldown) + worm hunt-state line; debug summon keys untouched.
- **`verify_worm` grows the Part 3 checks**; `tools/shoot_worm.gd` stays the breach evidence.

**Part 3 exit bar**
- [x] `verify_worm` (grown) passes: gated noise never charges the meter from the pad; sustained loud noise wakes the worm and it seeks the heard position; silence decays interest through PROWL → DEPART → despawn → zero dormant cost; a stalk tightens on fresh noise; a strike commits only onto swimmable ground, erupts the telegraph at the captured point, kills a stationary prey (signal asserted) and misses one displaced during the beat; the swallow → reload sequence restores the player. *(All 44 checks green 2026-08-23, twice — the second run after the telegraph fix. Wake took exactly the threshold's 300 loud steps; the woken worm entered 120 m out in SEEK; the miss-at-a-stale-point and the safe-ground stalk (never STRIKE, never inside the keep-out) both held; the kill cut control and returned it with the fade cleared.)*
- [x] `verify_prints`, ocean `--wetprints`, and the full battery stay green — the raise channel and everything else provably untouched. *(prints, wetprints (wet 0.65 / dry 0.33), clock, and player — player.gd changed, so its whole gate re-ran — all green 2026-08-23.)*
- [x] The whole loop reads at the gameplay camera: mound closing in, tightening circles, eruption, breach-strike — screenshots committed to `docs/references/`. *(`worm_hunt_{stalk,stalk_close,telegraph,strike,bite}.png` — the strike frame is a real dodge (eruption at the vacated point), the bite frame is the player standing inside the open maw as the screen darkens. Three shoot rounds: the telegraph needed scattering (mesa artifact, third sighting) plus a particle sputter to read — see DECISIONS.)*
- [x] Perf: no regression while a hunt is live; dormant stays zero. *(Worst deformation pass 0.315 ms over 7,648 passes across the whole grown gate; shoot frames 98–120 fps; dormant zero-cost asserted in the same run.)*
- [x] Zero warnings; standalone scenes clean; PLAN/DECISIONS/ARCHITECTURE updated. *(`--import` clean; per-script `--check-only` ×6 clean; world and screen_fade standalone runs clean.)*
- [ ] **Joshua has played the hunt** — fear pacing, fairness of the noise rule, rarity, dodge window all his to tune live. **← the one open item; closes Part 3 and the phase.**

**Handoff notes (2026-08-23, end of the Part 3 build session)**

The worm is a threat now, and the whole loop is live in the world scene. How to feel it without the wait: **F10 starts a hunt at once** (the director skips its meter); the honest path is to sprint around on deep sand until the meter crests — `F3` shows `director attraction NN%` climbing and the worm line once it wakes. **Crouch or stand still** and it loses you; **the courtyard, packed ground and the beach** are silent and unstrikable; when the sand starts spitting grains at your feet you have about 1.2 s to sprint anywhere else. Death is the swallow → black → your last save (or the courtyard if none).

Everything that will want live tuning is an export, in two places: **rarity and hearing** on `WormDirector` (`attraction_threshold` **60 and `calm_seconds` 45 are PLAYTEST values** — Joshua asked for easy waking while tuning, 2026-08-23; ~25 s of running wakes it and encounters can chain. Shipping rarity wants roughly 300+/240 back — re-raise when the feel locks. `attraction_decay` 0.6/s, `min_carry_depth` 0.5) and **the hunt's teeth** on `SandWorm`'s Hunting group (`strike_commit_distance` 14 — this ÷ swim speed IS the escape beat, `kill_radius` 3, `stalk_tighten_per_noise` 1.2, `lose_interest_seconds` 12, `prowl_seconds` 16, `hunt_spawn_distance` 120, `max_strike_misses` 2). The committed defaults are my eye; expect an audio-style live round. **The threat score is in (Joshua's sounds, 2026-08-23):** a hunt pauses the ambient music, plays his kraken wake-growl once, and loops his 100 BPM orchestral drums until the hunt ends; the breach roars with his attack growl. All wired through `worm.hunt_started`/`hunt_ended` → `MusicBed.set_hunt_active` and asserted by verify_worm. Still on the sourcing list: `worm_rumble_loop` (the distant-hunter warning) and `worm_telegraph_01` (the boil at your feet).

**Next session (queued by Joshua at session close, 2026-08-23): character animation & the death scene.** His framing: improve player animation, and give being consumed by the worm a real death animation — possibly blood. What the next session should know walking in:
- **The swallow sequence is deliberately plain today**: control cut, the body lerped toward the diving head, fade to black (`LevelRoot._on_swallowed` — ~30 lines, built to be replaced). The hooks a real death scene wants already exist: the mouth's three lip bones open (`get_mouth_open`), the head's position is the drag target, and `ScreenFade` is awaitable. A death *animation* (struggle pose, tumble into the maw) slots between the bite and the black without touching the worm.
- **This connects to the Phase 6 Part 2 clip backlog** (forward crouch-walk, real fall, landing, torch-carry — the Meshy sourcing list): one animation session could sweep both. The Phase 3 pipeline (`build_player.py`, AnimationTree, speed-matched clips) is the established path; a death clip joins it.
- **Blood is a palette conversation first, not a build call**: every color on screen is palette-law (ART_DIRECTION), the game's pillar is *cozy*, and the kill was designed as horror-by-suggestion (swallowed whole, screen to black). Options span from nothing → a dark sand-stain (existing `sand_shadow`) → a real red (a NEW palette color needing an ART_DIRECTION amendment). That's Joshua's ladder to pick from at the planning talk, not a default to assume.
- Phase numbering: queued as **6.9** on the status board; branch off the current chain when its planning session opens. Throwable noisemakers when we get there are one signal: emit `noise_made(position, loudness)` and connect it to the director.
