...
| 6.8 | The sand worm — desert mob | `feature/sand-worm` | 🟡 Parts 1–2 ✅; Part 3 planned with Joshua 2026-08-23 (noise wakes it · stalk→strike with an escape beat · swallow→reload save · safe ground + silence + sprint-to-cover all work · throwable noisemakers planned-for, not built) — build in progress this session. His look-sign-off folds into the Part 3 playtest. Merges after 6.7 |
...
**Part 3 exit bar**
- [ ] `verify_worm` (grown) passes: gated noise never charges the meter from the pad; sustained loud noise wakes the worm and it seeks the heard position; silence decays interest through PROWL → DEPART → despawn → zero dormant cost; a stalk tightens on fresh noise; a strike commits only onto swimmable ground, erupts the telegraph at the captured point, kills a stationary prey (signal asserted) and misses one displaced during the beat; the swallow → reload sequence restores the player.
- [ ] `verify_prints`, ocean `--wetprints`, and the full battery stay green — the raise channel and everything else provably untouched.
- [ ] The whole loop reads at the gameplay camera: mound closing in, tightening circles, eruption, breach-strike — screenshots committed to `docs/references/`.
- [ ] Perf: no regression while a hunt is live; dormant stays zero.
- [ ] Zero warnings; standalone scenes clean; PLAN/DECISIONS/ARCHITECTURE updated.
- [ ] **Joshua has played the hunt** — fear pacing, fairness of the noise rule, rarity, dodge window all his to tune live. **← closes Part 3 and the phase.**
