...
| 6.8 | The sand worm — desert mob | `feature/sand-worm` | 🟡 Parts 1–2 ✅ built & verified 2026-08-22 (titan body, breach, smoothed wake at 7.5 m/s). **Next session: Part 3 planning — when it appears, when it attacks, how to avoid (Joshua's framing)**; his formal look-sign-off folds into that playtest. Merges after 6.7 |
...
### Part 3 — The threat (its own planning session first — **the next session**)
...
**Queued 2026-08-22 (end of the Part 2 session, Joshua's framing): the next session is Part 3's planning conversation — "mechanics around the worm: when it appears, when it attacks, how to avoid."** The three questions map onto: the encounter director (when/where it spawns, how rarity works, whether night leans into it), the attack rules (what noise draws it, how the stalk telegraphs, when the strike commits — the mound and rumble are the telegraphs already built), and survivability (the safe-ground rule — packed pads and thin sand are physically worm-proof already — plus quiet-crouching, and whatever else the talk produces). Plus the deferred death-consequence call. Everything below the mechanics already exists: `summon_at` places the worm anywhere, `breach()` arcs it, the mouth bones open, `noise_made` fires with loudness per gait and nothing listens yet.
