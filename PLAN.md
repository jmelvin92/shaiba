...
Everything that will want live tuning is an export, in two places: **rarity and hearing** on `WormDirector` (`attraction_threshold` **60 and `calm_seconds` 45 are PLAYTEST values** — Joshua asked for easy waking while tuning, 2026-08-23; ~25 s of running wakes it and encounters can chain. Shipping rarity wants roughly 300+/240 back — re-raise when the feel locks. `attraction_decay` 0.6/s, `min_carry_depth` 0.5) and **the hunt's teeth** on `SandWorm`'s Hunting group (`strike_commit_distance` 14 — this ÷ swim speed IS the escape beat, `kill_radius` 3, `stalk_tighten_per_noise` 1.2, `lose_interest_seconds` 12, `prowl_seconds` 16, `hunt_spawn_distance` 120, `max_strike_misses` 2). The committed defaults are my eye; expect an audio-style live round. Two audio rows wait on sourcing (`worm_telegraph_01` joined `worm_rumble_loop`/`worm_breach_01` in `assets/audio/README.md` — the rumble matters most: right now the only warning of a distant hunter is the mound itself). Throwable noisemakers when we get there are one signal: emit `noise_made(position, loudness)` and connect it to the director.
