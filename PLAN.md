...
| 6.8 | The sand worm — desert mob | `feature/sand-worm` | 🟡 Part 1 built & verified 2026-08-22 — raise channel, wandering worm (F7/F8 debug), rumble hook, verify_worm + prints/wetprints green. **Awaiting Joshua: size-ladder pick + live mound look (F7).** Merges after 6.7; death design deferred to Part 3 planning |
...
- [ ] Size ladder shot from a fixed vantage; **Joshua picks the worm scale** — the pick closes Part 1 and sets Part 2's rig dimensions. **← the one open item: shots are committed at `docs/references/{a_tremors,b_greater,c_titan}_{mound,breach}.png` (9 m / 16 m / 26 m), awaiting his pick.**
