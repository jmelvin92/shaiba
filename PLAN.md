...
| 6.8 | The sand worm — desert mob | `feature/sand-worm` | 🟡 **Part 1 ✅ complete 2026-08-22** — raise channel, wandering worm (F7/F8 debug), rumble hook, all gates green; **scale picked: C titan, 26 m; kill will be swallow-whole**. Part 2 next, blocked on Joshua's model (rig contract 26 × 3.4 m). Merges after 6.7; death *consequence* still a Part 3 planning call |
...
**Rig contract from the Part 1 pick (2026-08-22): the worm is 26 m long × 3.4 m body diameter — rung C, the titan.** `tools/build_worm.py` conditions and rigs the model to those dimensions — a single spine chain (a worm is the simplest rig there is) driven **procedurally**: the body follows its burrow path and the bones bend to match, so breaches read correctly at any angle and speed with no baked-clip seams. Breach spectacle: the mesh arcing out of and back into the dune, sand burst at entry and exit (deformation stamp + the palette's language for thrown sand), breach audio hooks. The worm at this stage is ambient spectacle — rare, harmless, unforgettable.
