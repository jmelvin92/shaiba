...
- **The blender-mcp addon cannot run the glTF importer.** `bpy.ops.import_scene.gltf` dies inside the addon's execution context with `'Context' object has no attribute 'object'`. Use `execute_blender_code` for inspection and quick experiments, but do real asset work as a committed script run headless: `/Applications/Blender.app/Contents/MacOS/Blender --background --python tools/<name>.py`. `tools/build_player.py` (Phase 3) is the working pattern — it also makes the export reproducible, which the quality gates ask for. Blender in `--background` mode prints a harmless warning that the MCP server can't start; ignore it.
- Blender 5.2 API traps that cost a round each in Phase 3, both documented in `docs/ART_DIRECTION.md`: actions are *layered* so `action.fcurves` no longer exists, and `bpy_struct` returns a fresh wrapper per attribute access so `is`/`is not` comparisons on nodes, links and images are always wrong — use `==`.
- **macOS screenshot filenames contain a narrow no-break space (U+202F) before AM/PM**, not an ordinary space. `cp "Screenshot 2026-08-14 at 6.54.48 PM.png"` fails with "No such file" even though `ls` shows exactly that name. Use a glob (`~/Desktop/Screenshot\ 2026-08-14\ at\ 6.54.48*.png`) instead of typing the whole name.
- **To hand Joshua a running build**, launch detached and bring it forward, then leave it running: `nohup /Applications/Godot.app/Contents/MacOS/Godot --path . > <log> 2>&1 &` followed by the `osascript` focus call. Check `pgrep -fl Godot` and kill any previous instance first, or he tests a stale build — and re-launch after every asset rebuild, since the running process will not pick up a re-imported `.glb`.
- `sips -Z <px> -s format jpeg <in> --out <out>` is the quickest way to downscale screenshots for embedding in an artifact; full-size PNGs of the game window are ~2 MB each.
- **Windowed verify/screenshot runs are untrustworthy while any other Godot window exists** (e.g. a parallel session's build, or a leftover hand-off instance): the `osascript` focus dance targets "first process whose name contains Godot" and can promote the wrong window, leaving yours occluded — `ViewportTexture.get_image()` then reads stale black and checks fail with zeros. `pgrep -fl Godot` first and account for every instance; re-run before believing a failure. (Cost three phantom-failure rounds on 2026-08-14, see DECISIONS.)
